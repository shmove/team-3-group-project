...
namespace project.Interfaces {
    ...
    public abstract class BasePupilDataManager : IPupilDataManager, IInstallable{
        ...
        public Pupil[] GetTestCases(int Count) {
            ...
            for (int i = 0; i < Count; i++) PupilArray[i] = new Pupil(System.IO.Path.GetRandomFileName(), (v_Random.Next(0x00989680, 0x05f5e0ff) >> 0).ToString(), (v_Random.Next(0, 4) >> 1) > 1 ? "Tesco" : "Sainsbury's", v_Random.Next(0, 2) < 1, new List<Note>(){}, new List<TodoEntry>(){}, Pupil.YearGroups.S6, "Thing", DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss.s"), true);
            return PupilArray;
        }
